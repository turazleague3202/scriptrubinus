...
$podval_key = mysql_fetch_array(mysql_query('SELECT * FROM `podval_key` WHERE `clan` = "'.$clan['id'].'" '));
   if(!$podval_key) {
	mysql_query('INSERT INTO `podval_key` SET `clan` = "'.$clan['id'].'"');
	         exit(header('Location: /clan/podval/'));
}
?>
...
<div class="block center">
            <span class="btn_start"><span class="btn_end"><a class="btn" href="/clan/podval/">Завeршить сpажениe</a></span> </span>    </div>
		<?
...
<div class="dotted"></div>
<div class="block center">
            <span class="btn_start"><span class="btn_end"><a class="btn" href="/clan/podval/">Завepшить cpажениe</a></span> </span>    </div>
		<?		
			if($podval_b['images'] <= 16) {
...
if($user['level']>=60){
if($podval_b['images']==7){
mysql_query('UPDATE `users` SET `bos7` = `bos7` + 1 WHERE `id` = \''.$user['id'].'\'');
}
if($podval_b['images']==10){
mysql_query('UPDATE `users` SET `bos10` = `bos10` + 1 WHERE `id` = \''.$user['id'].'\'');
}
if($podval_b['images']==3){
mysql_query('UPDATE `users` SET `bossy3` = `bossy3` + 1 WHERE `id` = \''.$user['id'].'\'');
}
if($podval_b['images']==5){
mysql_query('UPDATE `users` SET `bossy5` = `bossy5` + 1 WHERE `id` = \''.$user['id'].'\'');
}
}

$clanbonys=round($podval_b['exp']*$clan['built_5']/70);
$gyl=60*$podval_b['images'];
$gok=120*$podval_b['images'];
mysql_query('UPDATE `clans` SET `s` = `s` + "'.($go*3).'", `exp` = `exp` + "'.($ex + $clan['built_5']).'", `g` = `g` + 5 WHERE `id` = "'.$clan['id'].'"');
mysql_query('UPDATE `users` SET `g` = `g` +"'.($go).'", `exp` = `exp` + "'.($ex+$clan['built_4']).'", `exepy` = `exepy` + "'.($ex+$clan['built_4']).'",`g` = `g` + "'.($gol).'" WHERE `id` = "'.$user['id'].'"');
mysql_query('UPDATE `users` SET `s` = `s` + "'.($podval_b['s']).'", `exp` = `exp` + "'.($podval_b['exp']+$clanbonys).'", `g` = `g` + 2 WHERE `id` = "'.$clan['id'].'"');
...
if($user['access'] ==3){
$_SESSION['err']=' <div class="alert"><div>                                     <div class="blue"><img src="/images/icons/reward.png" width="16" height="16" alt=""> Прибыль:</div>
 <img src="/images/icons/experience.png" width="16" height="16" alt=""> Авторитет:+'.($gok).'  <img src="/images/icons/silver.png" width="16" height="16" alt=""> Рубли +'.($gyl).'+ '.$clanbonys.'</div></div><div class="alert_bottom"></div>';
}
header('Location: /clan/podval/');
...
mysql_query('UPDATE `clan_memb` SET `exp` = `exp` + 1 WHERE `clan` = "'.$clan['id'].'" AND `user` = "'.$winer['user'].'"');
if($petsu){
...
<div class="clear"></div>
    ...
    <div class="a_separator"></div>
    <span class="btn_start"><span class="btn_end"><a class="btn" href="/clan/podval/?potion">Восстановить за     <img src="/images/icons/silver.png" width="16" height="16" alt=""> 84</a></span> </span></div></div><div class="alert_bottom"></div>';
		exit(header('Location: /clan/podval/'));
	}
...
mysql_query('INSERT INTO `podval_log` SET  `clan` = "'.$clan['id'].'",`time` = "'.time().'",`text` = "'.$ats.'",`user` = "'.$user['id'].'"');
	...
	?>
  <div class="content"><div class="block center color3 s125"><img src="/images/icons/circle.png" alt=""> <a href="/clan/podval/?'.$udet.'"><?=$podval_b['name']?></a><span class="white">,<span id="time_<?=($podval_b['time'] - time());?>000"><?=_time($podval_b['time'] - time());?></span>
</span></div>
...
<div class="line"></div>
	...
	<tbody><tr><td class="h-navig-item"><a href="/clan/podval/">Бой</a></td>
...
<div class="line"></div>
	  ...
	  <tbody>
	  <tr>
	  <td class="h-navig-item"><a href="/clan/podval/">Бой </a></td>
...
<div class="line"></div>
        ...
        exit(header('Location: /clan/podval/?chat=true'));		
		}
		?> <div class="block">
...
$bos_hp=100;
    ...
    <div class="dotted"></div>
<div class="block">        
    <div class="center">    <a href="?attack=true"><img class="center " src="/images/basement/<?=$podval_b['images']?>.jpg" width="50" height="50" alt=""></a>    </div>
...
<div class="clear"></div>
    ...
    <div class="block center">
        <span class="btn_start"><span class="btn_end"><a class="btn" href="?attack=true">Ηaнеcти yдар</a></span> </span>    </div>
    <div class="dotted"></div>
<div class="block">           
<?
...
<ul class="pagination"><li class="next"><?=pages('/podval/?');?></li></ul>
	<div class="dotted"></div>
<div class="menu"> <li><a href="?exit=1"><img src="/images/icons/cross.jpg" width="16" height="16" alt=""> Покинуть комнату </a></li></div><div class="dotted"></div>
</div>
...
echo'  <div class="content"><div class="block center color3 s125">Подвал</div>
		...
		$type = 4; 
			...
			?>
	    <div class="h-navig"><table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tbody><tr><td class="h-navig-item"><span class="active">лёгкие</span></td>
		<td class="h-navig-item"><a href="?complist=2">средние</a></td>
		<td class="h-navig-item"><a href="?complist=3">сложные</a></td>
		<td class="h-navig-item"><a href="?complist=4">сверх-сложные</a></td>
		</tr></tbody></table></div>
...
if($_GET['confirm'] == true && $clan_memb['rank'] > 3) {
...
mysql_query('INSERT INTO `clan_journal` SET `cl_id` = "'.$clan['id'].'",`time` = "'.$timey.'",`cl` = "3",`text` = "'.$texy.'",`user` = "'.$user['id'].'"');
	...
	exit(header('Location: /clan/podval/')); // Ок бой создан, теперь идем в бой!
}
...
<?
	...
	if($clan_memb['rank'] > 3 && $podval_key['key_'.$wiew['id'].''] > 0) {
		?>
...
' : '');?>
		<?
	}elseif($clan_memb['rank'] > 3 && $wiew['id'] == 1) {
				?>
<div class="dotted"></div>
        <div class="block center">
